...
from .models import (
    Event, Booking, Customer, Seat, Showtime, Tier, UniqueSeatBooking, City, Theatre, Screen, Transaction, Date
)
import uuid
import datetime
...
def get_shows(request):
    ...
    if theatre_id and event_id:
        ...
        show_list = [{"id": show.id, "time": show.slot_time.strftime("%I:%M %p")} for show in shows]
...
def get_seats(request):
    ...
    return JsonResponse({"seats": [{"id": seat.id, "seat_number": seat.seat_number} for seat in available_seats]})
...
def booking_success(request, booking_id):
    ...
    try:
        ...
        messages.error(request, "Error retrieving booking details.")
        return redirect("book_ticket")
